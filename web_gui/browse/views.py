...
def show_browse(request):
    ...
    if request.method == "POST":
        ...
        if "show_group" in request.POST:
            ...
            show_members = GroupAssignment.objects.filter(group_id = request.POST["show_group"]).order_by("-resemblence_rate")
            c.update({"show_group": show_group, "show_members": show_members})
            c.update(csrf(request))
            c.update({"browse_groups": True, "browse": False})
...
def show_browse_groups(request):
    ...
    if request.method == "POST":
        ...
        if "show" in request.POST:
            ...
            show_members = GroupAssignment.objects.filter(group_id = request.POST["show"]).order_by("-resemblence_rate")
            c.update({"show_group": show_group, "show_members": show_members})
            c.update(csrf(request))
            return render_to_response("show_group.html", c)
           
        if "show_member" in request.POST:
            # get sample info
            show_sample = Sample.objects.get(id = request.POST["show_member"])
            show_loops = Loop.objects.filter(sample = request.POST["show_member"])
            show_api = API.objects.filter(sample = request.POST["show_member"])
            show_hash = Hash.objects.get(sample = request.POST["show_member"])
            show_groups = GroupAssignment.objects.filter(member = request.POST["show_member"])
            c.update({"show_sample": show_sample, "show_loops": show_loops, "show_api": show_api, "show_hash": show_hash, "show_groups": show_groups})
            c.update({"browse_groups": False, "browse": True})
