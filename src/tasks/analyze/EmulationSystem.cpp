...
bool EmulationSystem::drawGraph(Graph *graph)
{
	...
	if(!QDir(m_graphsDir).exists()) {
		bool success = QDir().mkdir(m_graphsDir);
		if(!success) {
			LOG_ERROR("cannot create directory: [%s]\n", m_graphsDir.toStdString().c_str());
			LOG_ERROR("FAILURE\n\n");
			return false;
		}
	}
	...
	if(m_sample->info()->fileType() == "application/vnd.tcpdump.pcap")
		graphName = QString("%1/%2.png").arg(m_graphsDir).arg(Toolbox::pcapFlowBasename(m_sample->info()->name()));
	else
		graphName = QString("%1/%2.png").arg(m_graphsDir).arg(graphFile.baseName());
	...
	bool ret = system(dotCmd.toStdString().c_str());
	if(!ret) {
